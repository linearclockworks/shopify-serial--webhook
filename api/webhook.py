# Unified Webhook & Manual Processing Hub (Flask WSGI Entrypoint)
import json
import os
...
def calculate_line_item_discount(item: dict) -> float:
    """Calculates true per-unit dollar discount including order-level allocations."""
...
def queue_label_for_printing(sku, serial, is_cleartime=True):
    try:
        ...
        print(f"✓ Queued DYMO label job: {sku} | {sn_text}")
        return True
    except Exception as e:
...
def get_next_serial(key='global_serial_counter', prefix='LCK-'):
    ...
    if metafields:
        ...
        next_val = current + 1
        shopify_api_call(f'metafields/{metafield_id}.json', method='PUT', data={
            'metafield': {'id': metafield_id, 'value': str(next_val), 'type': 'number_integer'}
        })
        return serial
    return None
...
def create_sled_order_for_bryan(orig_order_number, sled_item_titles):
    try:
        ...
        for item_title in sled_item_titles:
            item_payload = {'quantity': 1, 'price': '0.00', 'title': f"Sled for {item_title}"}
            if sled_variant_id:
        ...
        new_order = {
            'order': {
                ...
                'applied_discount': {'title': '100% Sled Discount', 'value': '100.0', 'value_type': 'percentage'},
...
def execute_line_item_swap(order_id, old_line_item_id, new_variant_id, discount_amount=0.0, discount_description="Discount", currency_code="USD"):
    ...
    line_item_gids = [f"gid://shopify/LineItem/{old_line_item_id}", f"gid://shopify/CalculatedLineItem/{old_line_item_id}"]
    ...
    for item_gid in line_item_gids:
        res = shopify_graphql_call(remove_mutation, {"id": calc_order_id, "lineItemId": item_gid, "quantity": 0})
        if res and 'data' in res and res['data'].get('orderEditSetQuantity') and not res['data']['orderEditSetQuantity'].get('userErrors'):
            swap_success = True
            break

    if not swap_success:
        return False, "Line item removal failed."
    ...
    mutation orderEditAddVariant(\(id: ID!,\)variantId: ID!, $quantity: Int!) {
      orderEditAddVariant(id: \(id, variantId:\)variantId, quantity: $quantity) {
        calculatedOrder { 
          id 
          addedLineItems(first: 5) { edges { node { id } } }
        }
        userErrors { field message }
      }
    }
    ...
    if not res or 'errors' in res or not res.get('data', {}).get('orderEditAddVariant') or res['data']['orderEditAddVariant'].get('userErrors'):
        return False, "orderEditAddVariant failure"
    ...
    if discount_amount > 0:
        ...
        if added_edges:
            ...
            """
            shopify_graphql_call(discount_mutation, {
                "id": calc_order_id,
                "lineItemId": new_line_item_gid,
                "discount": {"description": discount_description, "fixedValue": {"amount": f"{discount_amount:.2f}", "currencyCode": currency_code}}
            })
...
def try_acquire_processing_lock(order_id):
    try:
        ...
        shopify_api_call(f'orders/{order_id}/metafields.json', method='POST', data={
            'metafield': {'namespace': 'webhook', 'key': 'processing_lock', 'value': datetime.now().isoformat(), 'type': 'single_line_text_field'}
        })
        return True
    except:
        return False
...
def mark_order_as_completed(order_id):
    try:
        shopify_api_call(f'orders/{order_id}/metafields.json', method='POST', data={
            'metafield': {'namespace': 'webhook', 'key': 'processing_completed', 'value': datetime.now().isoformat(), 'type': 'single_line_text_field'}
        })
        return True
    except:
        return False
...
def process_order(order_data, add_featured_tag=False, force=False):
    ...
    try:
        ...
        for item in order_data.get('line_items', []):
            ...
            if sku and sku.upper().startswith('LCK-'):
                ...
                if product_result:
                    ...
                    if 'featured' in tags_list or 'sample' not in tags_list:
                        continue
                else:
                    continue
                ...
                for i in range(current_qty):
                    ...
                    if new_product:
                        ...
                        execute_line_item_swap(order_id, line_item_id, new_product['variant_id'], discount_amount=unit_discount, discount_description=disc_desc, currency_code=currency_code)
        ...
        return {'status': 'success', 'order': order_number, 'products': products_created, 'lck_serials': lck_serials, 'cleartime_serials': cleartime_serials}
    except Exception as e:
