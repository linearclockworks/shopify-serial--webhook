...
def try_acquire_processing_lock(order_id):
    """Atomic locking mechanics matching webhook.py"""
    try:
...
def mark_order_as_completed(order_id):
    """Mark order completed with metafield tag matching webhook.py"""
    try:
...
def process_webhook(order_data):
    """Process the webhook order data with atomic duplicate locking"""
    order_id = order_data.get('id')
    order_number = order_data.get('name', '')
    
    if not try_acquire_processing_lock(order_id):
        print(f"⏭️ Processing lock exists for order {order_number} - skipping duplicate webhook run.")
        return {
    ...
    for item in order_data.get('line_items', []):
        ...
        if sku and is_cleartime and not product_title.startswith('--'):
            ...
            for i in range(current_qty):
                print(f"Generating cleartime serial {i+1} of {current_qty}...")
...
class handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        self.end_headers()
        self.wfile.write(b'Cleartime webhook handler is running - v3 (atomic locked)')
        return
